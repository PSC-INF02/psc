...
class Attribute:
	'''
	Reprensents an attribute to an entity.
	Since an attribute is generally associated to only one entity and entities have a list of attributes, an attribute will only consist in :
	...
	* A logical modifier (such as "not" for "not captain", no other example at the moment)
	...
	def modify(self, modifier):
		'''
		Amends the modifier of the attribute. Assuming an attribute can have at most one modifier, it erases the previous one.
