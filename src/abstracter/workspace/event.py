...
class Event:
	'''
	Represents an event, or a change, related in the text that we have somehow managed to find and want to push in the workspace. It contains:
	* An entity or attribute of origin (before the change)
	* An entity or attribute of destination (after the change)
	* A list of other events that this one is linked to. The links are labeled with the nature of the relationship between the two events (sameTime, consequence, etc.)
	'''
